...
struct LoadU8 {
	...
	static __m256i load8(const uint8_t *ptr)
	{
...
struct LoadU16 {
	...
	static __m256i load8(const uint16_t *ptr)
	{
		return _mm256_cvtepu16_epi32(_mm_load_si128((const __m128i *)ptr));
	}
};
...
inline FORCE_INLINE void depth_convert_avx2_impl(const void *src, void *dst, float scale, float offset, unsigned left, unsigned right)
{
	...
	if (left != vec_left) {
		__m256i x = Load::load8(src_p + vec_left - 8);
	...
	for (unsigned j = vec_left; j < vec_right; j += 8) {
		__m256i x = Load::load8(src_p + j);
		__m256 y = _mm256_cvtepi32_ps(x);
		y = _mm256_fmadd_ps(scale_ps, y, offset_ps);
		Store::store8(dst_p + j, y);
	}

	if (right != vec_right) {
		__m256i x = Load::load8(src_p + vec_right);
		__m256 y = _mm256_cvtepi32_ps(x);
		y = _mm256_fmadd_ps(scale_ps, y, offset_ps);
