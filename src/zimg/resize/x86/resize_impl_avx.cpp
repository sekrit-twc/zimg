...
public:
	...
	size_t get_tmp_size(unsigned left, unsigned right) const override
	{
		...
		try {
			checked_size_t size = (ceil_n(static_cast<checked_size_t>(range.second), 8) - floor_n(range.first, 8)) * sizeof(float) * 8;
			return size.get();
...
public:
	...
	{
		m_desc.step = 8;
		m_desc.scratchpad_size = (ceil_n(static_cast<checked_size_t>(filter.input_width), 8) * sizeof(float) * 8).get();
