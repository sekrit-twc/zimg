...
void test_case_ordered(const zimg::PixelFormat &pixel_in, const zimg::PixelFormat &pixel_out, const char *expected_sha1, double expected_snr)
{
	...
	auto result_lsx = zimg::depth::create_dither(dither, w, h, pixel_in, pixel_out, planes, zimg::CPUClass::LOONGARCH_LSX);

	graphengine::FilterValidation(result_lsx.filter_refs[0], { w, h, zimg::pixel_size(pixel_in.type) })
...
TEST(OrderedDitherLsxTest, test_ordered_dither_b2b)
{
	if (!has_lsx())
		GTEST_SKIP();
	...
	test_case_ordered(pixel_in, pixel_out, expected_sha1, INFINITY);
}

TEST(OrderedDitherLsxTest, test_ordered_dither_b2w)
{
	if (!has_lsx())
		GTEST_SKIP();
...
TEST(OrderedDitherLsxTest, test_ordered_dither_w2b)
{
	if (!has_lsx())
		GTEST_SKIP();
...
TEST(OrderedDitherLsxTest, test_ordered_dither_w2w)
{
	if (!has_lsx())
		GTEST_SKIP();
...
TEST(OrderedDitherLsxTest, test_ordered_dither_h2b)
{
	if (!has_lsx())
		GTEST_SKIP();
...
TEST(OrderedDitherLsxTest, test_ordered_dither_h2w)
{
	if (!has_lsx())
		GTEST_SKIP();
...
TEST(OrderedDitherLsxTest, test_ordered_dither_f2b)
{
	if (!has_lsx())
		GTEST_SKIP();
...
TEST(OrderedDitherLsxTest, test_ordered_dither_f2w)
{
	if (!has_lsx())
		GTEST_SKIP();
	...
	test_case_ordered(pixel_in, pixel_out, expected_sha1, 120.0);
}
