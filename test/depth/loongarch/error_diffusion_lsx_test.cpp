...
void test_case(const zimg::PixelFormat &pixel_in, const zimg::PixelFormat &pixel_out, const char *expected_sha1, double expected_snr)
{
	...
	zimg::depth::DepthConversion::result result_lsx = zimg::depth::create_dither(dither, w, h, pixel_in, pixel_out, planes, zimg::CPUClass::LOONGARCH_LSX);
	ASSERT_TRUE(result_c.filter_refs[0]);
	ASSERT_TRUE(result_lsx.filter_refs[0]);
	ASSERT_TRUE(assert_different_dynamic_type(result_c.filter_refs[0], result_lsx.filter_refs[0]));
...
TEST(ErrorDiffusionLsxTest, test_error_diffusion_b2b)
{
...
TEST(ErrorDiffusionLsxTest, test_error_diffusion_b2w)
{
	zimg::PixelFormat pixel_in{ zimg::PixelType::BYTE, 8, true, false };
...
TEST(ErrorDiffusionLsxTest, test_error_diffusion_w2b)
{
...
TEST(ErrorDiffusionLsxTest, test_error_diffusion_w2w)
{
...
TEST(ErrorDiffusionLsxTest, test_error_diffusion_h2b)
{
...
TEST(ErrorDiffusionLsxTest, test_error_diffusion_h2w)
{
	zimg::PixelFormat pixel_in = zimg::PixelType::HALF;
...
TEST(ErrorDiffusionLsxTest, test_error_diffusion_f2b)
{
...
TEST(ErrorDiffusionLsxTest, test_error_diffusion_f2w)
{
	zimg::PixelFormat pixel_in = zimg::PixelType::FLOAT;
	zimg::PixelFormat pixel_out = zimg::PixelType::WORD;
