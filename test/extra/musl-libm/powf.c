...
#define T my__powf_log2_data.tab
#define A my__powf_log2_data.poly
...
#define C __exp2f_data.poly_scaled
	...
	ki = converttoint(xd);
#else
#define C my__exp2f_data.poly
#define SHIFT my__exp2f_data.shift_scaled
...
float _mypowf(float x, float y)
{
	...
	if (predict_false((asuint64(ylogx) >> 47 & 0xffff) >=
		...
		if (ylogx > 0x1.fffffffd1d571p+6 * POWF_SCALE)
			return my__math_oflowf(sign_bias);
		if (ylogx <= -150.0 * POWF_SCALE)
			return my__math_uflowf(sign_bias);
	}
