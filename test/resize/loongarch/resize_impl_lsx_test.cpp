               ...
               const zimg::PixelFormat &format, const char *expected_sha1, double expected_snr)
{
	...
	auto filter_lsx = builder.set_cpu(zimg::CPUClass::LOONGARCH_LSX).create();

	ASSERT_TRUE(assert_different_dynamic_type(filter_c.get(), filter_lsx.get()));

	graphengine::FilterValidation validation(filter_lsx.get(), { src_w, src_h, zimg::pixel_size(format.type) });
...
TEST(ResizeImplLsxTest, test_resize_h_u10)
{
...
TEST(ResizeImplLsxTest, test_resize_h_u16)
{
	const unsigned src_w = 640;
	const unsigned dst_w = 960;
	const unsigned h = 480;
...
TEST(ResizeImplLsxTest, test_resize_v_u10)
{
...
TEST(ResizeImplLsxTest, test_resize_v_u16)
{
	const unsigned w = 640;
	const unsigned src_h = 480;
	const unsigned dst_h = 720;
...
TEST(ResizeImplLsxTest, test_resize_h_f32)
{
	const unsigned src_w = 640;
	const unsigned dst_w = 960;
	const unsigned h = 480;
...
TEST(ResizeImplLsxTest, test_resize_v_f32)
{
	const unsigned w = 640;
	const unsigned src_h = 480;
	const unsigned dst_h = 720;
