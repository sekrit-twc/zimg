	           ...
	           const zimg::PixelFormat &format, const char *expected_sha1, double expected_snr)
{
	...
	ASSERT_TRUE(assert_different_dynamic_type(filter_c.get(), filter_neon.get()));

	graphengine::FilterValidation(filter_neon.get(), { src_w, src_h, zimg::pixel_size(format.type) })
