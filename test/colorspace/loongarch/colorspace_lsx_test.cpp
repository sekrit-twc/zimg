               ...
               const char * const expected_sha1[3], double expected_snr)
{
	...
	ASSERT_TRUE(filter_lsx);

	graphengine::FilterValidation(filter_lsx.get(), { w, h, zimg::pixel_size(zimg::PixelType::FLOAT) })
...
TEST(ColorspaceConversionLSXTest, test_matrix)
{
...
TEST(ColorspaceConversionLSXTest, test_transfer_lut)
{
	using namespace zimg::colorspace;
